...
export async function toDataUri(url: string): Promise<string> {
  ...
  const localDataUri = FARM_ASSET_DATA_URIS[url];
  if (localDataUri) {
    cache.set(url, localDataUri);
    return localDataUri;
  }

  // Keep any future local URL as a same-origin fallback instead of making
  // an invalid server-side fetch.
  if (url.startsWith("/")) {
    cache.set(url, url);
    return url;
  }
...
export async function avatarToDataUri(avatarUrl: string): Promise<string> {
  if (avatarUrl.startsWith("/")) return toDataUri(avatarUrl);
  // 追加 size 参数以控制下载尺寸
