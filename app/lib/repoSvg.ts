...
const ICONS = {
  book: FARM_ASSETS.greenhouse,
  star: FARM_ASSETS.sunflower,
  trident: FARM_ASSETS.fence,
  spider_eye: FARM_ASSETS.scarecrow,
  map: FARM_ASSETS.watering,
};
...
function guiShell(w: number, h: number): string {
  return `
<rect x="0" y="0" width="${w}" height="${h}" fill="${FARM_PALETTE.ink}" />
<rect x="4" y="4" width="${w - 8}" height="${h - 8}" fill="${FARM_PALETTE.paper}" />
<polygon points="4,4 ${w - 4},4 ${w - 8},8 8,8 8,${h - 8} 4,${h - 4}" fill="#fff8df" />
<polygon points="${w - 4},${h - 4} ${w - 4},4 ${w - 8},8 ${w - 8},${h - 8} 8,${h - 8} 4,${h - 4}" fill="${FARM_PALETTE.paperDark}" />`;
}

// ===== 凹陷槽 =====
function sunkenSlot(x: number, y: number, w: number, h: number): string {
  return `
<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="#3e3027" />
<polygon points="${x},${y} ${x + w},${y} ${x + w - 4},${y + 4} ${x + 4},${y + 4} ${x + 4},${y + h - 4} ${x},${y + h}" fill="#5a4030" />
<polygon points="${x + w},${y + h} ${x + w},${y} ${x + w - 4},${y + 4} ${x + w - 4},${y + h - 4} ${x + 4},${y + h - 4} ${x},${y + h}" fill="#fff8df" />`;
}

// ===== 徽章 (Public / Private) =====
function badge(label: string, x: number, y: number, w: number, h: number): string {
  return `
<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${FARM_PALETTE.leaf}" />
<polygon points="${x},${y} ${x + w},${y} ${x + w - 4},${y + 4} ${x + 4},${y + 4} ${x + 4},${y + h - 4} ${x},${y + h}" fill="#214536" />
<polygon points="${x + w},${y + h} ${x + w},${y} ${x + w - 4},${y + 4} ${x + w - 4},${y + h - 4} ${x + 4},${y + h - 4} ${x},${y + h}" fill="#fff8df" />`;
}
...
export function generateRepoSvg(params: RepoSvgParams): string {
  ...
  const titleTspans = isTruncated
    ? `<tspan fill="${FARM_PALETTE.ink}">${escapeXml(truncated)}</tspan>`
    : `<tspan fill="#72543f">${safeOwner} / </tspan><tspan fill="${FARM_PALETTE.ink}">${safeRepo}</tspan>`;
...
<g class="anim-fade" style="animation-delay: 0.1s">
  ...
  <text x="430" y="29" class="mc-font" font-size="14" fill="#fff8df" text-anchor="middle" filter="url(#shadow-dark)">${badgeLabel}</text>
</g>

<!-- 中部：仓库描述 -->
<g class="anim-fade" style="animation-delay: 0.2s">
  <text x="18" y="60" class="mc-font" font-size="16" font-weight="bold" fill="${FARM_PALETTE.ink}">${line1}</text>
  ${line2 ? `<text x="18" y="79" class="mc-font" font-size="16" font-weight="bold" fill="${FARM_PALETTE.ink}">${line2}</text>` : ""}
</g>
...
<g class="anim-fade" style="animation-delay: 0.3s">
  ...
  <g transform="translate(26, 119)">
    ...
    <text x="14" y="12" class="mc-font" font-size="16" fill="#72543f" filter="url(#shadow-dark)">${escapeXml(language)}</text>
  </g>
  <!-- Stars -->
  <g transform="translate(${starsGroupX}, 119)">
    <image href="${ICONS.star}" x="0" y="0" width="16" height="16" filter="url(#shadow-dark)" />
    <text x="20" y="12" class="mc-font" font-size="16" fill="${FARM_PALETTE.water}" filter="url(#shadow-dark)">${stars.toLocaleString()}</text>
  </g>
  <!-- Forks -->
  <g transform="translate(${forksGroupX}, 119)">
    <image href="${ICONS.trident}" x="0" y="0" width="16" height="16" filter="url(#shadow-dark)" />
    <text x="20" y="12" class="mc-font" font-size="16" fill="${FARM_PALETTE.leaf}" filter="url(#shadow-dark)">${forks.toLocaleString()}</text>
  </g>
  <!-- Issues -->
  <g transform="translate(${issuesGroupX}, 119)">
    <image href="${ICONS.spider_eye}" x="0" y="0" width="16" height="16" filter="url(#shadow-dark)" />
    <text x="20" y="12" class="mc-font" font-size="16" fill="${FARM_PALETTE.berry}" filter="url(#shadow-dark)">${issues.toLocaleString()}</text>
  </g>
  <!-- Size (右锚定) -->
  <g transform="translate(${sizeGroupX}, 119)">
    <image href="${ICONS.map}" x="0" y="0" width="16" height="16" filter="url(#shadow-dark)" />
    <text x="24" y="12" class="mc-font" font-size="16" fill="${FARM_PALETTE.gold}" filter="url(#shadow-dark)">${sizeText}</text>
  </g>
</g>
</svg>`;
}
...
export async function generateBakedRepoSvg(params: RepoSvgParams): Promise<string> {
  ...
  const titleFg = isTruncated
    ? bakeMixedTextElement({ text: safeTruncated, x: 48, y: 33, fontSize: 20, fill: FARM_PALETTE.ink, fontWeight: "bold", filter: "url(#shadow-title)" })
    : bakeMixedTextElement({ text: `${safeOwner} / `, x: 48, y: 33, fontSize: 20, fill: "#72543f", fontWeight: "bold", filter: "url(#shadow-title)" }) +
      bakeMixedTextElement({ text: safeRepo, x: 48 + ownerWidth, y: 33, fontSize: 20, fill: FARM_PALETTE.ink, fontWeight: "bold", filter: "url(#shadow-title)" });
  ...
  const desc1Fg = bakeMixedTextElement({ text: line1, x: 18, y: 60, fontSize: 16, fill: FARM_PALETTE.ink, fontWeight: "bold" });
  const desc2Fg = line2 ? bakeMixedTextElement({ text: line2, x: 18, y: 79, fontSize: 16, fill: FARM_PALETTE.ink, fontWeight: "bold" }) : "";
  ...
  const langPath = bakeMixedTextElement({ text: escapeXml(language), x: slotLeft + 14, y: 131, fontSize: 16, fill: "#72543f", filter: "url(#shadow-dark)" });
  ...
  const sizePath = bakeTextElement({ text: sizeText, x: sizeTextX, y: 131, fontSize: 16, fill: FARM_PALETTE.gold, filter: "url(#shadow-dark)" });
  ...
  const starsPath = bakeTextElement({ text: stars.toLocaleString(), x: starsX + 20, y: 131, fontSize: 16, fill: FARM_PALETTE.water, filter: "url(#shadow-dark)" });
  const forksPath = bakeTextElement({ text: forks.toLocaleString(), x: forksX + 20, y: 131, fontSize: 16, fill: FARM_PALETTE.leaf, filter: "url(#shadow-dark)" });
  const issuesPath = bakeTextElement({ text: issues.toLocaleString(), x: issuesX + 20, y: 131, fontSize: 16, fill: FARM_PALETTE.berry, filter: "url(#shadow-dark)" });
